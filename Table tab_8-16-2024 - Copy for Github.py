...
import pandas as pd
from sqlalchemy import create_engine, MetaData, Table, select #To connect to the PostgreSQL database and query the data from the database
from sqlalchemy.orm import sessionmaker #To create a session to connect to the PostgreSQL database
from datetime import datetime
import os #To use the environment variables
import time #To use the time library
import plotly.graph_objects as go #To use the Plotly library to create the graph
...
BankingData = Table('concentration_of_banking', metadata, autoload_with=engine)
...
# Fetch the unique quarters from the database
def get_quarters():
    query = select(BankingData.c['Quarter']).distinct()
    results = session.execute(query).fetchall()
    quarters = [row[0] for row in results]
    quarters.sort(key=quarter_to_date)
    options = [{'label': q, 'value': q} for q in quarters]
    latest_quarter = quarters[-1] if quarters else None
    return options, latest_quarter

options, latest_quarter = get_quarters()
...
def update_table(bank_name, select_quarter, assets_range):
    query = select(
        BankingData.c['Bank Name'],
        BankingData.c['Quarter'],
        BankingData.c['Date'],
        BankingData.c['Consolidated Assets'],
        BankingData.c['Percentage of Total Cosolidated Assets'],
        BankingData.c['Bank ID'],
        #BankingData.c['Bank Location'],
        #BankingData.c['Domestic Branches'],
        #BankingData.c['Foreign Branches']
    )
    
    if bank_name:
        query = query.where(BankingData.c['Bank Name'].ilike(f'%{bank_name}%'))
    
    if select_quarter:
        query = query.where(BankingData.c['Quarter'] == select_quarter)
    
    if assets_range:
        query = query.where(BankingData.c['Consolidated Assets'].between(assets_range[0], assets_range[1]))

    query = query.order_by(BankingData.c['Consolidated Assets'].desc())
    results = session.execute(query).fetchall()
    ...
    df_table_dir=r"C:\Directory\to\df_table"
    df_table.to_csv(df_table_dir + r"\banking_assets_data.csv", index=False)
...
df_ai = pd.read_csv(r"C:\Directory\to\banking_assets_data.csv") #Load the dataset
df_ai_5=df_ai.head(5) #Grab the first 5 rows
csv_string=df_ai_5.to_string(index=False) #Convert the first 5 rows to a string
